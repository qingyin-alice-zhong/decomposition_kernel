...
void conv2d_3x768_16x16_fp32(float input[3072], float output[16], float param[3076]) {
    ...
    constexpr int STRIDE_H = 16;
    constexpr int STRIDE_W = 16;
    ...
    for (int oc = 0; oc < OUT_CHANNELS; ++oc) {
        for (int oh = 0; oh < OUTPUT_HEIGHT; ++oh) {
            const int ih_base = oh * STRIDE_H;
            for (int ow = 0; ow < OUTPUT_WIDTH; ++ow) {
                const int iw_base = ow * STRIDE_W;
                ...
                const int out_idx = (oc * OUTPUT_HEIGHT + oh) * OUTPUT_WIDTH + ow;
                out[out_idx] = static_cast<float>(acc);
            }
        }
    }
