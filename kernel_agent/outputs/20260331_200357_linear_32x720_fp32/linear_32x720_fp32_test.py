...
PARAM_SIZE = OUT_TILE * IN_FEATURES + OUT_TILE
...
def run_tiled_linear(mod, x: np.ndarray, ref: np.ndarray) -> np.ndarray:
    ...
    for n in range(FULL_BATCH):
        for s in range(FULL_SEQ):
            ...
            for out_start in range(0, OUT_FEATURES, OUT_TILE):
                ...
                param[:out_extent] = ref[n, s, out_start:out_start + out_extent]

                out_tile = np.zeros((OUTPUT_SIZE,), dtype=np.float32)
                mod(in_vec, out_tile, param)
                output[n, s, out_start:out_start + out_extent] = out_tile[:out_extent]
                tile_count += 1
...
def _test_linear_32x720_fp32(kernel_path: str):
    print(
        ...
        f"shape=(1,50,32)->(1,50,720), out_tile={OUT_TILE}"
    )
    ...
    if "MLIR_AIE_INSTALL_DIR" in os.environ:
        ...
        for seed in seeds:
            for case_name in cases:
                ...
                out = run_tiled_linear(mod, x, ref)
                total_cases += 1
                ...
                if seed == seeds[0] and case_name == cases[0] and repeat_count > 1:
                    ...
                    for rep in range(repeat_count - 1):
                        out_repeat = run_tiled_linear(mod, x, ref)
