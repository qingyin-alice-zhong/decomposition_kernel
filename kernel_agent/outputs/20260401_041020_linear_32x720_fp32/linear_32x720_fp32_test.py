...
PARAM_SIZE = ABI_TILE * IN_FEATURES + ABI_TILE
...
def build_case(seed: int, case_name: str):
    ...
    if case_name == "normal":
        ...
        reps = (FULL_BATCH * FULL_SEQ * IN_FEATURES + len(base) - 1) // len(base)
        x = np.tile(base, reps)[: FULL_BATCH * FULL_SEQ * IN_FEATURES].reshape(FULL_BATCH, FULL_SEQ, IN_FEATURES)
    else:
    ...
    bias = (rng.standard_normal((OUT_FEATURES,), dtype=np.float32) * 0.02).astype(np.float32)
    ...
    return x.astype(np.float32, copy=False), weight, bias, ref


def _build_top(kernel_path: str):
    linear_kernel = ExternalModule(
        top="linear_32x720_fp32",
...
def run_tiled_linear(mod, x: np.ndarray, weight: np.ndarray, bias: np.ndarray) -> np.ndarray:
    output = np.zeros((FULL_BATCH, FULL_SEQ, OUT_FEATURES), dtype=np.float32)
    seq_sample_raw = os.environ.get("ALLO_SEQ_SAMPLE", "0,1,2,3,4,5,10,15,20,25,30,40,49")
    seq_indices = sorted(
        {
            idx
    ...
    for n in range(FULL_BATCH):
        for s in seq_indices:
            ...
            for out_start in range(0, OUT_FEATURES, OUT_TILE):
                ...
                bias_tile = np.zeros((ABI_TILE,), dtype=np.float32)
                param[:IN_FEATURES] = weight[out_start, :]
                bias_tile[:out_extent] = bias[out_start:out_start + out_extent]
                param[IN_FEATURES:] = bias_tile

                out_vec = np.zeros((OUTPUT_SIZE,), dtype=np.float32)
                mod(in_vec, out_vec, param)
                output[n, s, out_start:out_start + out_extent] = out_vec[:out_extent]
                tile_count += 1

    print(
        f"Executed {tile_count} tiles for linear_32x720_fp32 "
...
def _test_linear_32x720_fp32(kernel_path: str):
    print(
        "Running tiled validation for linear_32x720_fp32: "
        f"shape=(1,50,32)->(1,50,720), out_tile={OUT_TILE}, abi_tile={ABI_TILE}"
    )
    ...
    if "MLIR_AIE_INSTALL_DIR" in os.environ:
        ...
        for seed in seeds:
            for case_name in cases:
                x, weight, bias, ref = build_case(seed=seed, case_name=case_name)
                ...
                if seed == seeds[0] and case_name == cases[0] and repeat_count > 1:
                    ...
                    for rep in range(repeat_count - 1):
                        out_repeat, _ = run_tiled_linear(mod, x, weight, bias)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--kernel_path", type=str, default="linear_32x720_fp32.cc")
    args = parser.parse_args()

    if Path(TOP_PRJ_ABS_DIR).exists():
        shutil.rmtree(TOP_PRJ_ABS_DIR)

    _test_linear_32x720_fp32(args.kernel_path)
