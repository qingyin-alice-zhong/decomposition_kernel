...
FULL_RUN = os.environ.get("ALLO_FULL_RUN", "0") == "1"
MAX_GROUPS = int(os.environ.get("ALLO_MAX_GROUPS", "8"))
STRICT_VERIFY = os.environ.get("ALLO_STRICT_VERIFY", "0") == "1"
NUMERIC_RTOL = 1e-2
NUMERIC_ATOL = 1e-2
...
def run_tiled_reference(mod, full_input: np.ndarray, full_weight: np.ndarray, full_bias: np.ndarray, ref_output: np.ndarray):
    ...
    coverage = np.zeros((FULL_BATCH, FULL_OUT_CHANNELS, FULL_OUTPUT_HEIGHT, FULL_OUTPUT_WIDTH), dtype=bool)
