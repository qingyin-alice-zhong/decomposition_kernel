...
def reference_abs_int8(x: Annotated[np.ndarray, "shape: (1024,)"]) -> Annotated[np.ndarray, "shape: (1024,)"]:
    return np.abs(x).astype(np.int8)
# Reference code ends


def _test_abs_int8(kernel_path: str):
    abs_kernel = ExternalModule(
        top="abs_int8",
    ...
    if "MLIR_AIE_INSTALL_DIR" in os.environ:
        ...
        # ===== Analyze trace via shared utility if generated yiunder top.prj/ =====
        analyze_trace(top_prj_dir=TOP_PRJ_ABS_DIR, targetname="abs_int8", colshift=1)
...
if __name__ == "__main__":
    ...
    _test_abs_int8(args.kernel_path)
