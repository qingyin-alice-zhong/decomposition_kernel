...
def build_position_case(case_name: str, seed: int = 0) -> np.ndarray:
    if case_name == "sequential":
        ...
        ids = ((np.arange(FULL_SEQ, dtype=np.int32) * 997) % VOCAB_SIZE).astype(np.int32)
    elif case_name == "boundary_mix":
        base = np.array([0, VOCAB_SIZE - 1, 1, VOCAB_SIZE - 2, VOCAB_SIZE // 2, VOCAB_SIZE // 2 + 1], dtype=np.int32)
...
def run_tiled_embedding(mod, position_ids: np.ndarray, weight: np.ndarray) -> np.ndarray:
    ...
    print(f"Executed {tile_count} tiles for embedding_49280x960_fp32")
    return output
...
def _test_embedding_49280x960_fp32(kernel_path: str):
    print(
        "Running tiled validation for embedding_49280x960_fp32: "
    ...
    repeat_count = int(os.environ.get("ALLO_REPEAT_COUNT", "3"))
    perf_max_min_us = float(os.environ.get("ALLO_PERF_MAX_MIN_US", "0"))
    perf_max_avg_us = float(os.environ.get("ALLO_PERF_MAX_AVG_US", "0"))
    ...
    if "MLIR_AIE_INSTALL_DIR" in os.environ:
        ...
        if perf_max_min_us > 0:
            try:
                measured_min_us = float(getattr(mod, "min_time", 0.0))
                if measured_min_us > perf_max_min_us:
                    failures.append(
                        f"performance(min_time) exceeded threshold: {measured_min_us:.3f}us > {perf_max_min_us:.3f}us"
                    )
            except Exception as e:
                failures.append(f"performance(min_time) check failed: {e}")

        if perf_max_avg_us > 0:
            try:
                measured_avg_us = float(getattr(mod, "average_time", 0.0))
                if measured_avg_us > perf_max_avg_us:
                    failures.append(
                        f"performance(average_time) exceeded threshold: {measured_avg_us:.3f}us > {perf_max_avg_us:.3f}us"
                    )
            except Exception as e:
                failures.append(f"performance(average_time) check failed: {e}")
        ...
        for seed in seeds:
            ...
            for case_name in cases:
                ...
                try:
                    np.testing.assert_allclose(output, ref, rtol=NUMERIC_RTOL, atol=NUMERIC_ATOL)
                except AssertionError as e:
                    failures.append(f"seed={seed}, case={case_name}: {str(e)}")
                    continue

                max_abs, mean_abs = compute_error_stats(output, ref)
                ...
                if seed == seeds[0] and case_name == cases[0] and repeat_count > 1:
                    ...
                    for rep in range(repeat_count - 1):
                        ...
                        if not np.allclose(output_repeat, output, rtol=0.0, atol=0.0):
                            ...
                            break
                        try:
                            np.testing.assert_allclose(output_repeat, ref, rtol=NUMERIC_RTOL, atol=NUMERIC_ATOL)
                        except AssertionError as e:
                            failures.append(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--kernel_path", type=str, default="embedding_49280x960_fp32.cc")
    args = parser.parse_args()

    if Path(TOP_PRJ_ABS_DIR).exists():
        shutil.rmtree(TOP_PRJ_ABS_DIR)

    _test_embedding_49280x960_fp32(args.kernel_path)
