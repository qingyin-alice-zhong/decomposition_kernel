...
int main(int argc, const char *argv[]) {
  ...
  auto bo_in0 = xrt::bo(device, 972 * sizeof(float),
  ...
  if (ifile0_size != 972 * sizeof(float)) {
    std::cerr << "Error: Invalid input file, byte number mismatch.\n";
    return 1;
  }
  std::vector<float> vec0(972);
  ifile0.read(reinterpret_cast<char*>(vec0.data()), 972 * sizeof(float));
  srcVec0.insert(srcVec0.end(), vec0.begin(), vec0.end());
  memcpy(bufIn0, srcVec0.data(), (srcVec0.size() * sizeof(float)));

  auto bo_out1 = xrt::bo(device, 2048 * sizeof(float),
                        XRT_BO_FLAGS_HOST_ONLY, kernel.group_id(4));
  auto bo_in2 = xrt::bo(device, 224 * sizeof(float),
  ...
  if (ifile2_size != 224 * sizeof(float)) {
    std::cerr << "Error: Invalid input file, byte number mismatch.\n";
    return 1;
  }
  std::vector<float> vec2(224);
  ifile2.read(reinterpret_cast<char*>(vec2.data()), 224 * sizeof(float));
  ...
  ofile1.write(reinterpret_cast<const char*>(bufOut1 + 0), 2048 * sizeof(float));
  ofile1.close();
  ifile2.close();
  if (trace_size > 0) {
