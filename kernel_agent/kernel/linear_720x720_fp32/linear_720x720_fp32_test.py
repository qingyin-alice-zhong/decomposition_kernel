...
def run_tiled_linear(mod, x: np.ndarray, weight: np.ndarray, bias: np.ndarray):
    ...
    print(
        f"Executed {tile_count} tiles for linear_720x720_fp32 "
...
def _test_linear_720x720_fp32(kernel_path: str):
    print(
        "Running tiled validation for linear_720x720_fp32: "
        f"shape=(1,50,720)->(1,50,720), out_tile={OUT_TILE}, abi_tile={ABI_TILE}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--kernel_path", type=str, default="linear_720x720_fp32.cc")
    args = parser.parse_args()

    if Path(TOP_PRJ_ABS_DIR).exists():
        shutil.rmtree(TOP_PRJ_ABS_DIR)

    _test_linear_720x720_fp32(args.kernel_path)
