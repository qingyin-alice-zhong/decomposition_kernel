...
KERNEL_NAME = "linear_768x768_fp32"
FULL_BATCH = 1
FULL_SEQ = 1024
IN_FEATURES = 768
OUT_FEATURES = 768
...
def build_case(seed: int, case_name: str):
    ...
    bias = (rng.standard_normal((OUT_FEATURES,), dtype=np.float32) * 0.02).astype(np.float32) if HAS_BIAS else np.zeros((OUT_FEATURES,), dtype=np.float32)

    with torch.no_grad():
        bias_tensor = torch.from_numpy(bias) if HAS_BIAS else None
        ...
        for s in seq_indices:
            ...
            for out_start in range(0, OUT_FEATURES, OUT_TILE):
                ...
                for in_start in range(0, IN_FEATURES, KERNEL_IN_FEATURES):
                    ...
                    if HAS_BIAS and in_start == 0:
                        param[KERNEL_IN_FEATURES] = bias[out_start]

                    out_vec = np.zeros((OUTPUT_SIZE,), dtype=np.float32)
                    mod(in_chunk, out_vec, param)
                    partial_sum[0] += out_vec[0]
                    tile_count += 1

                output[n, s, out_start] = partial_sum[0]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--kernel_path", type=str, default="linear_768x768_fp32.cc")
