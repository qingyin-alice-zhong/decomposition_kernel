...
def _build_case(seed: int, case_name: str):
    ...
    return x, F.silu(torch.from_numpy(x)).numpy().astype(np.float32)


def _build_top(kernel_path: str):
    k = ExternalModule(top="silu_fp32", impl_path=kernel_path, input_idx=[0], output_idx=[1])

    @df.region()
    def top(A: float32[720], C: float32[720]):
        @df.kernel(mapping=[1], args=[A, C])
        def core(a: float32[720] @ Ly, c: float32[720] @ Ly):
            k(a, c)
...
def _test(kernel_path: str):
    ...
    for seed in seeds:
        for case_name in cases:
            ...
            print(
                f"[CASE PASS] seed={seed}, case={case_name}, sampled_seq={seq_indices}, "
...
if __name__ == "__main__":
    ...
    parser.add_argument("--kernel_path", type=str, default="silu_fp32.cc")
