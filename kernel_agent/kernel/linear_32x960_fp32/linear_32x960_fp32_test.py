...
def run_tiled_linear(mod, x: np.ndarray, weight: np.ndarray, bias: np.ndarray) -> np.ndarray:
    ...
    print(
        f"Executed {tile_count} tiles for linear_32x960_fp32 "
...
def _test_linear_32x960_fp32(kernel_path: str):
    print(
        "Running tiled validation for linear_32x960_fp32: "
        f"shape=(1,50,32)->(1,50,960), out_tile={OUT_TILE}, abi_tile={ABI_TILE}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--kernel_path", type=str, default="linear_32x960_fp32.cc")
    args = parser.parse_args()

    if Path(TOP_PRJ_ABS_DIR).exists():
        shutil.rmtree(TOP_PRJ_ABS_DIR)

    _test_linear_32x960_fp32(args.kernel_path)
