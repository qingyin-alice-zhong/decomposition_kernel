...
def run_linear(mod, x: np.ndarray, weight: np.ndarray, bias: np.ndarray) -> np.ndarray:
        ...
        for s in seq_indices:
            ...
            for out_start in range(0, OUT_FEATURES, OUT_TILE):
                ...
                for in_start in range(0, IN_FEATURES, KERNEL_IN_FEATURES):
                    ...
                    param[:KERNEL_IN_FEATURES] = weight_chunk.reshape(-1)
                    if in_start == 0:
                        param[KERNEL_IN_FEATURES:] = bias[out_start:out_start + out_extent]
    ...
    print(
        f"Executed {tile_count} tiles for linear_720x32_fp32 "
...
def _test_linear_720x32_fp32(kernel_path: str):
    print(
        "Running tiled validation for linear_720x32_fp32: "
        f"shape=(1,50,720)->(1,50,32), out_tile={OUT_TILE}, abi_tile={ABI_TILE}"
    )
    ...
    if "MLIR_AIE_INSTALL_DIR" in os.environ:
        ...
        for seed in seeds:
            for case_name in cases:
                ...
                out, seq_indices = run_linear(mod, x, weight, bias)
                total_cases += 1
                ...
                if seed == seeds[0] and case_name == cases[0] and repeat_count > 1:
                    ...
                    for rep in range(repeat_count - 1):
                        out_repeat, _ = run_linear(mod, x, weight, bias)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--kernel_path", type=str, default="linear_720x32_fp32.cc")
    args = parser.parse_args()

    if Path(TOP_PRJ_ABS_DIR).exists():
        shutil.rmtree(TOP_PRJ_ABS_DIR)

    _test_linear_720x32_fp32(args.kernel_path)
